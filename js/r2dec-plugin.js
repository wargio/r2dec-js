...
function decompile_offset(architecture, fcnname) {
  ...
  if (data.graph && data.graph.length > 0) {
    ...
    Shared.context.printLog(
      "Error: Please analyze the " + fcnname + " first.",
      true,
    );
  } else {
    Shared.context.printLog(
    ...
    if (architecture) {
      ...
      if (Shared.evars.extra.allfunctions) {
        ...
        functions.forEach(function (x) {
          ...
          Shared.context.printLine(
            Shared.printer.theme.comment(
              "/* name: " + x.name + " @ 0x" + x.offset.toString(16) + " */",
            ),
            x.offset,
          );
        ...
        Shared.context.printLine(
          Shared.printer.theme.comment(
            "/* r2dec pseudo code output" + r2version + " */",
          ),
          current,
        );
        Shared.context.printLine(
          Shared.printer.theme.comment("/* " + Shared.evars.extra.file + " */"),
          current,
        );
      ...
      errors.push(
        Shared.evars.arch + " is not currently supported.\n" +
