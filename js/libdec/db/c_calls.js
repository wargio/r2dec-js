    ...
    fcn: function (bits) {
      ...
      this.print = function (offset) {
        ...
        Global().context.printLine(
          Global().context.identfy() + t.types(type) + " " + t.callname(call) +
            " (" + args + ") {",
          offset,
        );
        for (i = 0; i < this.data.length; i++) {
          Global().context.printLine(
            Global().context.identfy() +
              a(this.data[i].replace(/###/g, this.bits)),
            offset,
          );
        }
  ...
  bit_mask: {
    ...
    fcn: function () {
      ...
      this.print = function (offset) {
        ...
        var t = Global().printer.theme;
        Global().context.printLine(
          Global().context.identfy() + t.macro(this.mask),
          offset,
        );
      };
    },
    ...
    fcn: function (bits) {
      ...
      this.print = function (offset) {
        ...
        for (var i = 0; i < this.data.length; i++) {
          Global().context.printLine(
            Global().context.identfy() + t.macro(this.data[i]),
            offset,
          );
        }
      };
