...
import Cpp from "../db/cpp.js";
import CCalls from "../db/c_calls.js";
...
var _generic_asm = function (asm) {
  ...
  this.toString = function () {
    return Global().printer.theme.callname("__asm") + " (" +
      Global().printer.theme.text('"' + this.asm + '"') + ")";
  };
};
...
var _cast = function (source, type) {
  return new _uexpr(
    parenthesize(Global().printer.theme.types(type)) + " ",
    source,
  );
};

var _generic_call = function (function_name, args) {
  this.function_name = Extra.is.string(function_name)
    ? Cpp(Extra.replace.call(function_name))
    : function_name;
  this.callargs = args || [];
...
var _generic_method_call = function (
  object_this,
  function_name,
  args,
  method_sep,
  ...
  this.object_this = Extra.is.string(object_this)
    ? Cpp(object_this)
    : object_this;
  this.function_name = Extra.is.string(function_name)
    ? Cpp(function_name)
  ...
  this.toString = function () {
    ...
    return this.prefix + this.object_this + this.method_sep +
      [fname, parenthesize(this.callargs.join(", "))].join(" ") + this.postfix;
  };
};

var _generic_rotate = function (
  destination,
  source_a,
  rotation,
  bits,
  ...
  this.rotation = Extra.is.number(rotation)
    ? ("0x" + rotation.toString(16))
    : rotation;
...
var _assign_object_field = function (
  destination,
  object,
  field,
  method_sep,
...
var _generic_objc_call = function (
  object_this,
  function_name,
  function_arguments,
) {
...
export default {
  ...
  conditional_assign: function (
    ...
    return _assign(
      destination,
      new _texpr("?", ":", condition.toString(), src_true, src_false),
    );
  },
  conditional_math: function (
    destination,
    source_a,
    source_b,
    cond,
  ...
  bit_mask: function (destination, source_a, source_b) {
    ...
    return _assign(
      destination,
      new _generic_call("BIT_MASK", [source_a, source_b]),
    );
  },
  /* MEMORY */
  read_memory: function (pointer, register, bits, is_signed) {
    var value = (Extra.is.string(register) || Extra.is.number(register))
      ? Variable.local(register.toString(), Extra.to.type(bits, is_signed))
      : register;
    var ptr = (Extra.is.number(pointer) || Long.isLong(pointer))
      ? Variable.pointer(
        "0x" + pointer.toString(16),
        Extra.to.type(bits, is_signed),
      )
      : pointer;
  ...
  method_call: function (
    ...
    return new _generic_method_call(
      object_this,
      function_name,
      function_arguments,
      method_separator,
  ...
  /* Object based langs */
  assign_to_object_field: function (
    destination,
    object,
    method_separator,
    field,
  ) {
    return new _assign_object_field(
      destination,
      object,
      field,
      method_separator,
  ...
  assign_from_object_field: function (
    destination,
    object,
    method_separator,
    field,
