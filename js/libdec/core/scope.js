...
function _print_locals(locals, address, spaced) {
  ...
  for (var i = 0; i < locals.length; i++) {
    var local = Extra.is.string(locals[i])
      ? a(locals[i])
      : locals[i].toString(true);
    Global().context.printLine(
      Global().context.identfy() + local + ";",
      address,
    );
  }
...
function _print_block_data(block) {
  if (Global().evars.honor.blocks) {
    ...
    Global().context.printLine(
      ident + t.comment("/* address 0x" + addr + " */"),
      block.address,
    );
  }
}
...
export default {
  brace: function (address) {
    ...
    this.toString = function () {
      return "}" +
        (__debug
          ? Global().printer.theme.comment(" // 0x" + this.address.toString(16))
          : "");
    };
    this.print = function () {
      Global().context.identOut();
      var offset = Global().evars.honor.offsets
        ? Extra.align_address(this.address)
        : "";
      Global().context.printLine(
        Global().context.identfy(
          offset.length,
          Global().printer.theme.integers(offset),
        ) + this.toString(),
        this.address,
      );
    };
  ...
  routine: function (address, extra) {
    ...
    this.toString = function () {
      ...
      var t = Global().printer.theme;
      var a = Global().printer.auto;
      return t.types(e.returns) + " " + t.callname(e.routine_name) + " (" +
        e.args.map(function (x) {
          return Extra.is.string(x) ? a(x) : x.toString(true);
        }).join(", ") + ") {" +
        (__debug ? t.comment(" // 0x" + this.address.toString(16)) : "");
    };
    this.print = function () {
      var e = this.extra;
      var t = Global().printer.theme;
      _print_locals(e.globals, this.address, true);
      var asmname = Global().evars.honor.offsets
        ? Extra.align_address(this.address)
        : "; (fcn) " + e.name + " ()";
  ...
  if: function (address, condition, locals) {
    ...
    this.toString = function () {
      var t = Global().printer.theme;
      return t.flow("if") + " (" + this.condition + ") {" +
        (__debug ? t.comment(" // 0x" + this.address.toString(16)) : "");
    };
    this.print = function () {
      var offset = Global().evars.honor.offsets
  ...
  do: function (address, locals) {
    ...
    this.print = function () {
      Global().context.printLine(
        Global().context.identfy() + this.toString(),
        this.address,
      );
      Global().context.identIn();
      _print_block_data(this);
  ...
  while: function (address, condition, locals) {
    ...
    this.toString = function () {
      var t = Global().printer.theme;
      return t.flow("while") + " (" + this.condition + ") {" +
        (__debug ? t.comment(" // 0x" + this.address.toString(16)) : "");
    };
    this.print = function () {
      var offset = Global().evars.honor.offsets
