...
var _asm_radare2_view = function (instr, ascodeline) {
  ...
  if (instr.code && instr.code.composed) {
    if (instr.cond && instr.jump) {
      s += (instr.jump.gt(instr.location) ? "if" : "while") + " (" +
        _conditional(instr.cond) + ") {\n";
    }
    ...
    s += "if (" + _conditional(instr.cond) + ") goto 0x" +
      instr.jump.toString(16);
  }
  ...
  if (Global().evars.honor.offsets) {
    ...
    if (instr.code && instr.code.composed) {
      Global().context.printLine(
        Global().context.identfy(addr.length, t.integers(addr)) +
          instr.code.composed[0] + ";",
        instr.location,
      );
      for (i = 1; i < instr.code.composed.length; i++) {
        Global().context.printLine(
          Global().context.identfy(addr.length, t.integers(addr)) +
            instr.code.composed[i] + ";",
          instr.location,
        );
      }
    } else if (_printable(instr)) {
      Global().context.printLine(
        Global().context.identfy(addr.length, t.integers(addr)) + instr.code +
          ";",
        instr.location,
      );
    }
    ...
    if (instr.code && instr.code.composed) {
      Global().context.printLine(
        Global().context.identfy(
          s,
          t.integers(addr) + " " + b(instr.simplified),
        ) + instr.code.composed[0] + ";",
        instr.location,
      );
      for (i = 1; i < instr.code.composed.length; i++) {
        Global().context.printLine(
          Global().context.identfy() + instr.code.composed[i] + ";",
          instr.location,
        );
      }
    } else {
    ...
    if (instr.code && instr.code.composed) {
      ...
      Global().context.printLine(
        Global().context.identfy() + instr.code + ";",
        instr.location,
      );
    }
  }
...
var _instruction = function (data, arch, marker) {
  ...
  this.location = Long.from(data.offset, true);
  this.assembly = data.disasm || data.opcode;
  this.simplified = data.opcode;
  this.cpp_type = this.assembly.match(
    /(class|method|struct)\s[\w:]+(<[\w:<, >]+>)?\s+/,
  );
  if (this.cpp_type) {
  ...
  if (Global().evars.honor.xrefs) {
    for (var i = 0; i < this.xrefs.length; i++) {
      var e = "XREF " + this.xrefs[i].type + ": 0x" +
        this.xrefs[i].addr.toString(16);
  ...
  this.print = function () {
    ...
    if (this.comments.length == 1) {
      Global().context.printLine(
        empty + t.comment("/* " + this.comments[0] + " */"),
        this.location,
      );
      ...
      for (var i = 1; i < this.comments.length; i++) {
        var comment = " * " + this.comments[i] +
          (i == this.comments.length - 1 ? " */" : "");
        Global().context.printLine(empty + t.comment(comment), this.location);
      }
    }
    if (this.label) {
      Global().context.printLine(
        Global().context.identfy(null, null, true) + this.label + ":",
        this.location,
      );
    }
    _asm_view(this);
