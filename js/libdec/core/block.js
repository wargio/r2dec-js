...
Bounds.invalid = function () {
  return new Bounds(Long.MAX_U64_VALUE, Long.MAX_U64_VALUE);
};
...
export default function _block(bounds) {
  ...
  this.lastHead = function (extra) {
    return this.extraHead.length > 0
      ? this.extraHead[this.extraHead.length - 1]
      : null;
  };
  ...
  this.print = function () {
    ...
    for (i = 0; i < this.instructions.length; i++) {
      while (
        this.extraHead[h] &&
        this.extraHead[h].address.lte(this.instructions[i].location)
      ) {
      ...
      while (
        this.extraTail[t] &&
        this.extraTail[t].address.eq(this.instructions[i].location)
      ) {
