  ...
  parse: function (assembly) {
    ...
    /*
		   Regex explaination:
			...
			$
		*/
    var tokens = assembly.match(
      /^(\w+)\.?([wlb])?([sn])?\/?(hi|eq|hs|pl|pz|str)? ?(((@)?((\(\w+\s\w+\))|(-)?([\w._]+)(\+)?)))? ?(((@)?((\(\w+\s\w+\))|(-)?([\w._]+)(\+)?)))?$/,
    );
    var mnem = tokens[1];
    ...
    var opd = [{
      ...
      increment: tokens[12] == "+",
      token: tokens[11] ||
        (tokens[9] ? tokens[9].replace(/\((\w+)\s(\w+)\)/g, "$2 + $1") : null),
      toString: function () {
