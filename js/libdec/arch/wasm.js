...
var _set_global = function (instr, context, instructions, type) {
  ...
  if (!context.global[n]) {
    context.global[n] = new WasmVar(
      "global_" + instr.parsed.opd[0],
      type,
      instr,
    );
  }
...
var _remove_const = function (instr, instructions, stackval) {
  if (
    stackval &&
    instructions.indexOf(stackval.instr) == (instructions.indexOf(instr) - 1) &&
    stackval.instr.parsed.mnem == "const"
  ) {
...
var _wasm_arch = {
  instructions: {
    ...
    get_local: function (instr, context, instructions) {
      context.stack.push(
        _set_local(instr, context, instructions, "int32_t", true),
      );
      return Base.nop();
    },
    ...
    return: function (instr, context, instructions) {
      ...
      if (context.stack.length > 0) {
        if (context.stack.length > 1) {
          Global().warning(
            "[wasm] stack len is not zero: " + context.stack.length,
          );
        }
  ...
  localvars: function (context) {
    ...
    for (var k in context.local) {
      ...
      if (
        Extra.is.inObject(context.input, o) ||
        Extra.is.inObject(context.global, o)
      ) {
        continue;
      }
