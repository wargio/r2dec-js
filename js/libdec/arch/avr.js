...
const AVR_X_MACRO = "#define " + AVR_X_MACRONAME +
  " ((uint16_t*)((r27 << 8) | r26))";
const AVR_Y_MACRO = "#define " + AVR_Y_MACRONAME +
  " ((uint16_t*)((r29 << 8) | r28))";
const AVR_Z_MACRO = "#define " + AVR_Z_MACRONAME +
  " ((uint16_t*)((r31 << 8) | r30))";
const AVR_MEMORY = {
  "x": {
...
export default {
  instructions: {
    ...
    bset: function (instr, context) {
      ...
      _returns_r0(instr, context);
      return Base.macro(
        "SREG_SET_BIT (" + instr.parsed.opd[0] + ")",
        "#define SREG_SET_BIT(x) __asm(bset (x))",
      );
    },
    ...
    clt: function (instr) {
      ...
      return Base.macro(
        "CLEAR_TRANSFER_FLAG",
        "#define CLEAR_TRANSFER_FLAG __asm(clt)",
      );
    },
    ...
    cpc: function (instr, context, instructions) {
      ...
      if (
        instructions[p - 1] &&
        instructions[p - 1].parsed.mnem.indexOf("cp") == 0
      ) {
    ...
    iret: function (instr) {
      return Base.macro(
        ...
        "#define RETURN_FROM_INTERRUPT __asm(iret)",
      );
    },
    ...
    ldd: function (instr, context) {
      ...
      //pointer, register, bits, is_signed
      var op = Base.read_memory(
        AVR_MEMORY[ptr[0]].name + offset,
        instr.parsed.opd[0],
        8,
        false,
      );
    ...
    rcall: function (instr, context) {
      _returns_r0(instr, context);
      return Base.call(
        Variable.functionPointer(instr.parsed.opd[0], AVR_MEM_BITS),
        [],
      );
    },
    ...
    reti: function (instr) {
      return Base.macro(
        "RETURN_FROM_INTERRUPT",
        "#define RETURN_FROM_INTERRUPT __asm(reti)",
      );
    },
    ...
    sbrc: function (instr, context, instructions) {
      ...
      if (next) {
        _compare(
          "(" + instr.parsed.opd[0] + " & (1 << " + instr.parsed.opd[1] + "))",
          "0",
          context,
        );
        context.cond.instr = instr;
    ...
    set: function (instr, context) {
      ...
      return Base.macro(
        "SET_TRANSFER_FLAG",
        "#define SET_TRANSFER_FLAG __asm(set)",
      );
    },
    st: _store16,
