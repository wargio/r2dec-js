...
var _load_bi = function(instr, bits, signed) {
	...
	var ops = [
		...
		Base.increase(e.opd[1], e.opd[2] || '0')
	];
...
export default {
	instructions: {
		...
		beqzs8: function(instr, context) {
			instr.conditional('$a5', '0', 'EQ');
			return Base.nop();
		},
		bnezs8: function(instr, context) {
			instr.conditional('$a5', '0', 'NE');
			return Base.nop();
		},
		beqs38: function(instr, context) {
			instr.conditional(_arg(instr, 0), '$a5', 'EQ');
			return Base.nop();
		},
		bnes38: function(instr, context) {
			instr.conditional(_arg(instr, 0), '$a5', 'NE');
			return Base.nop();
		},
