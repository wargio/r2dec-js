...
var _fix_missing_jump = function (instr) {
  ...
  if (!instr.jump) {
    instr.jump = Long.from(address, true, address.startsWith("0x") ? 16 : 10);
  }
};
...
var _fix_arg = function (instr) {
  ...
  return instr.string
    ...
    : Variable.local(instr.parsed.opd[0], _register_size(instr.parsed.opd[0]));
  //}
  ...
  for (i = instrs.length - 1; i >= 0 && nargs > 0; i--) {
    ...
    if (mnem === "adrp") {
      ...
      arg = instrs[i].string
        ...
        : Variable.local(opd2, _register_size(opd2));
    ...
    if (mnem === "push") {
      arg = instrs[i].string
        ? Variable.string(instrs[i].string)
        : Variable.local(opd1, _register_size(opd1));
...
var _populate_arm64_call_args = function (instrs, nargs, context, varargs) {
  ...
  for (i = instrs.length - 1; i >= 0 && (nargs > 0 || varargs); i--) {
    ...
    if (
      op == "pop" || op.startsWith("cb") || op.startsWith("b") ||
      op.startsWith("tb")
    ) {
      break;
    }
...
var _guess_cdecl_nargs = function (instrs, context) {
  ...
  for (var i = instrs.length - 1; i >= 0; i--) {
    ...
    if (
      (mnem.startsWith("st") || mnem.startsWith("ldr") ||
        mnem.startsWith("mov")) && _is_stack_reg(opd1)
    ) {
      nargs++;
...
function _get_return_value_register(instr, instructions, args) {
  ...
  if (
    nextinstr && ["mov r7, r7", "mov x29, x29"].indexOf(nextinstr.assembly) >= 0
  ) {
...
var _call = function (instr, context, instructions) {
  ...
  if (ObjC.is(callname)) {
    ...
    if (!ObjC.is_class_method(callname)) {
      ...
      var subslice = instructions.slice(
        (current - 8) < 0 ? 0 : current - 8,
        current,
      );
      args = [receiver, selector].map(function (reg) {
        ...
        if (marker && marker[reg]) {
          ...
          vtype = marker[reg].instr.parsed.mnem.startsWith("ld")
            ? "pointer"
            : "local";
          ret = marker[reg].instr.string || marker[reg].instr.klass;
          if (!ret) {
            ret = Variable[vtype](
              "0x" + marker[reg].value.toString(16),
              Extra.to.type(bits, false),
            );
          }
        ...
        for (i = subslice.length - 1; i >= 0; i--) {
          ...
          if (
            reg == subslice[i].parsed.opd[0] &&
            ("mov" == subslice[i].parsed.mnem ||
              ("ldr" == subslice[i].parsed.mnem &&
                subslice[i].parsed.opd[1].startsWith &&
                subslice[i].parsed.opd[1].startsWith("str_")))
          ) {
            subslice[i].valid = false;
            opd2 = subslice[i].parsed.opd[1];
            if (
              (opd2.startsWith("str.") || opd2.startsWith("str_")) &&
              !subslice[i].string
            ) {
              return Global().xrefs.find_string(
                opd2.replace(/^str_/, "str."),
              ) || opd2.substr(4);
            }
      ...
      for (j = 0; j < pargs.length; j++) {
        ...
        for (i = subslice.length - 1; i >= 0; i--) {
          if (
            !subslice[i].parsed.mnem ||
            subslice[i].parsed.mnem.startsWith("b") || subslice[i].jump
          ) {
            break;
          }
    ...
    args = populate_call_args(
      instructions.slice(0, current),
      nargs,
      context,
      varargs,
    );
  ...
  if (callname.match(/^[rwx]\d+$/) || callname.match(/^0x[a-fA-F\d]+$/)) {
    callname = Variable.functionPointer(
      callname,
      _reg_bits[callname[0]] || 0,
      args,
    );
  }
...
var _arm_conditional_execution = function (condition, p) {
  var f = (function () {
    var that = function (instr, context, instructions) {
      _conditional_inline(instr, context, instructions, that.condition);
      return that.instruction(instr, context, instructions);
    };
    return that;
  })();
...
var _arm_ret = function (instr, context, instructions) {
  ...
  if (["r0", "w0", "x0"].indexOf(instructions[start - 1].parsed.opd[0]) >= 0) {
    ...
    if (
      context.markers[instr.marker]["r0"] &&
      context.markers[instr.marker]["r0"].instr.valid
    ) {
      //context.markers[instr.marker]['r0'].instr.valid = false;
      returnval = "0x" + context.markers[instr.marker]["r0"].value.toString(16);
    } else if (
      context.markers[instr.marker]["w0"] &&
      context.markers[instr.marker]["w0"].instr.valid
    ) {
      //context.markers[instr.marker]['w0'].instr.valid = false;
      returnval = "0x" + context.markers[instr.marker]["w0"].value.toString(16);
    } else if (
      context.markers[instr.marker]["x0"] &&
      context.markers[instr.marker]["x0"].instr.valid
    ) {
...
var _arm = {
  instructions: {
    ...
    bx: function (instr, context, instructions) {
      ...
      if (callname == "lr") {
        ...
        if (
          ...
          (instructions[start - 1].parsed.opd[0] == "r0" ||
    ...
    ldr: function (instr, context) {
      ...
      if (typeof opd2 == "string" && opd2.startsWith("str_")) {
        instr.string =
          Global().xrefs.find_string(opd2.replace(/^str_/, "str.")) ||
          opd2.replace("str_", "");
    ...
    movt: function (instr, context, instructions) {
      ...
      return Base.special(
        dst + " = (" + dst + " & 0xFFFF) | 0x" + src.toString(16) + "0000",
      );
    },
    movw: function (instr, context) {
      ...
      return Base.special(
        dst + " = (" + dst + " & 0xFFFF0000) | (" + instr.parsed.opd[1] +
          " & 0xFFFF)",
      );
    },
    ...
    "push.w": function () {},
    ror: function (instr) {
      return Base.rotate_right(
        instr.parsed.opd[0],
        instr.parsed.opd[1],
        parseInt(instr.parsed.opd[2], 16).toString(),
        32,
      );
    },
    rol: function (instr) {
      return Base.rotate_left(
        instr.parsed.opd[0],
        instr.parsed.opd[1],
        parseInt(instr.parsed.opd[2], 16).toString(),
        32,
    ...
    bfxil: function (instr, context, instructions) {
      ...
      var mask_A = Long.MAX_U64_VALUE.shl(lsb + width);
      ...
      var sp = opds[0] + " = (uint" + bits + "_t) (" + opds[0] + " & 0x" +
        mask_A.toString(16);
    ...
    rev: function (instr) {
      return Base.swap_endian(
        instr.parsed.opd[0],
        instr.parsed.opd[1],
        _reg_bits[instr.parsed.opd[0][0]],
      );
    },
    ...
    fcvt: function (instr) {
      return Base.cast(
        instr.parsed.opd[0],
        instr.parsed.opd[1],
        instr.parsed.opd[0][0] == "s" ? "float" : "double",
      );
    },
    invalid: function () {
      return Base.nop();
  ...
  parse: function (asm, orig) {
    ...
    if (radare2 && orig.match(/ldr \w\d+.*\[pc, 0x[\da-fA-F]+\]/)) {
      asm = orig;
...
function _apply_new_assign(register, data) {
  if (data && data.instr.parsed.mnem) {
    if (data.instr.string) {
      data.instr.code = Base.assign(
        data.instr.parsed.opd[0],
        Variable.string(data.instr.string),
      );
      ...
      data.instr.code = Base.read_memory(
        "0x" + data.value.toString(16),
        data.instr.parsed.opd[0],
        32,
        false,
      );
...
var _apply_math = {
  ...
  movk: function (marker, instr) {
    ...
    var s = Long.from(parseInt(instr.parsed.opd[1]).toString(16), true, 16);
    ...
    instr.string = Global().xrefs.find_string(
      marker[instr.parsed.opd[0]].value,
    );
    instr.symbol = Global().xrefs.find_symbol(
      marker[instr.parsed.opd[0]].value,
    );
  },
  movw: function (marker, instr) {
    ...
    _apply_new_assign(instr.parsed.opd[0], marker[instr.parsed.opd[0]]);
    marker[instr.parsed.opd[0]] = {
      value: Long.from(s.toString(16), true, 16),
      instr: instr,
    };
  },
  movt: function (marker, instr) {
    ...
    i.valid = false;
    marker[instr.parsed.opd[0]].instr = instr;
    marker[instr.parsed.opd[0]].value = v.or(
      Long.from(s.toString(16) + "0000", true, 16),
    );
    instr.string = Global().xrefs.find_string(
      marker[instr.parsed.opd[0]].value,
    );
  ...
  orr: function (marker, instr) {
    ...
    marker[instr.parsed.opd[0]].instr = instr;
    marker[instr.parsed.opd[0]].value = Long.from(s.toString(16), true, 16);
  },
  add: function (marker, instr) {
    if (
      instr.parsed.opd.length != 2 && instr.parsed.opd[0] != instr.parsed.opd[1]
    ) {
      return;
    } else if (
      !marker[instr.parsed.opd[0]] ||
      (instr.parsed.opd.length == 2 && instr.parsed.opd[1] != "pc")
    ) {
      return;
    }
    if (
      instr.parsed.opd.length != 2 && !marker[instr.parsed.opd[2]] &&
      !instr.parsed.opd[2].startsWith("0x")
    ) {
      _apply_new_assign(instr.parsed.opd[0], marker[instr.parsed.opd[0]]);
      delete marker[instr.parsed.opd[0]];
      return;
    ...
    if (instr.parsed.opd[1] == "pc") {
      ...
      var value = Long.from(
        parseInt(instr.parsed.opd[2]).toString(16),
        true,
        16,
      );
  ...
  ldr: function (marker, instr) {
    ...
    if (
      isarr && instr.parsed.opd[1][0] != "pc" && !marker[instr.parsed.opd[1][0]]
    ) {
      return;
    } else if (
      !isarr && !marker[instr.parsed.opd[1]] &&
      !instr.parsed.opd[1].startsWith("0x")
    ) {
      return;
    }
    var number, v;
    if (instr.parsed.opd[1][0] == "pc") {
      ...
      instr.string = Global().xrefs.find_string(v) ||
        Global().xrefs.find_string(number);
      instr.symbol = Global().xrefs.find_symbol(v) ||
        Global().xrefs.find_symbol(number);
    } else if (!isarr && instr.parsed.opd[1].startsWith("0x")) {
      number = Long.from(parseInt(instr.parsed.opd[1]).toString(16), true, 16);
      v = _value_at(number);
      instr.string = Global().xrefs.find_string(v) ||
        Global().xrefs.find_string(number);
      instr.symbol = Global().xrefs.find_symbol(v) ||
        Global().xrefs.find_symbol(number);
      instr.klass = Global().xrefs.find_class(v) ||
        Global().xrefs.find_class(number);
    } else {
      var data = marker[instr.parsed.opd[1]] || marker[instr.parsed.opd[1][0]];
      if (
        instr.parsed.opd[0] == instr.parsed.opd[1] ||
        instr.parsed.opd[0] == instr.parsed.opd[1][0]
      ) {
      ...
      instr.klass = Global().xrefs.find_class(_value_at(v)) ||
        Global().xrefs.find_class(_value_at(_value_at(number.add(4)))) ||
        Global().xrefs.find_class(v) || Global().xrefs.find_class(number);
    }
    if (instr.string) {
      instr.code = Base.assign(
        instr.parsed.opd[0],
        Variable.string(instr.string),
      );
