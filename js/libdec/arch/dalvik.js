...
function arg_usage(value, type, context) {
  if (
    !value.startsWith("0x") && !context.objects[value] &&
...
function _invoke_static(instr, context, instructions) {
  ...
  var args = p.args.map(function (x) {
    return _handle_type(
      context.objects[x],
      Variable.local(x, instr.bits, true),
    );
  });
...
function _invoke_direct(instr, context, instructions) {
  ...
  if (context.arguments[p.args[0]]) {
    ...
    dst = object.name;
    method = Extra.replace.object(instr.parsed.opd[0]).replace(
      object.type + ".",
      "",
    );
...
function _invoke_super(instr, context, instructions) {
  ...
  if (!context.objects[p.args[0]]) {
    arg_usage(
      p.args[0],
      Extra.replace.object(p.opd[0]).replace(/(\.?.+)\..+$/, "$1"),
      context,
    );
    method = "super()" +
      Extra.replace.object(p.opd[0]).replace(/\.?.+(\..+$)/, "$1");
...
function _aget_generic(instr, context, instructions) {
  ...
  var index = _handle_type(
    context.objects[instr.parsed.opd[2]],
    Variable.local(instr.parsed.opd[2], instr.bits, true),
  );
...
function _iget_generic(instr, context, instructions) {
  ...
  arg_usage(
    p.opd[1],
    Extra.replace.object(p.opd[2]).replace(/^(\.?.+)\..+$/, "$1"),
    context,
  );
...
function _generic64_math3(instr, context, type, operation) {
  var dst = Variable.local(
    next_reg_join(instr.parsed.opd[0], ":"),
    instr.bits,
    true,
  );
  var src1 = Variable.local(
    next_reg_join(instr.parsed.opd[1], ":"),
    instr.bits,
    true,
  );
  arg_usage(instr.parsed.opd[1], type, context);
  ...
  if (!instr.parsed.opd[2]) {
    ...
    arg_usage(instr.parsed.opd[2], type, context);
    src2 = instr.parsed.opd[2]
      ? Variable.local(
        next_reg_join(instr.parsed.opd[2], ":"),
        instr.bits,
        true,
      )
      : src1;
...
function _get_type(value) {
  ...
  var pos = ["[Z", "[B", "[S", "[C", "[I", "[J", "[F", "[D", "[V"].indexOf(
    value.toUpperCase(),
  );
  if (pos >= 0) {
    return [
...
export default {
  instructions: {
    ...
    "const-string": function (instr, context, instructions) {
      ...
      context.objects[p.opd[0]] = {
        instr: instr,
        type: !instr.string && p.opd[1].startsWith("0x")
          ? DalvikType.Number
          : DalvikType.String,
      };
      var dst = Variable.local(p.opd[0], instr.bits, true);
    ...
    "filled-new-array": function (instr, context, instructions) {
      ...
      var src = Variable.newarray(
        _get_type(instr.parsed.opd[0]),
        size,
        instr.parsed.args,
      );
      context.objects[instr.parsed.opd[0]] = {
    ...
    "new-array": function (instr, context, instructions) {
      var size = _handle_type(
        context.objects[instr.parsed.opd[1]],
        Variable.local(instr.parsed.opd[1], instr.bits, true),
      );
      var dst = Variable.local(instr.parsed.opd[0], instr.bits, true);
    ...
    "if-eqz": function (instr, context, instructions) {
      ...
      instr.jump = !instr.jump
        ? Long.from(instr.parsed.opd[1], true, 16)
        : instr.jump;
      return Base.nop();
    },
    ...
    "if-eq": function (instr, context, instructions) {
      ...
      instr.jump = !instr.jump
        ? Long.from(instr.parsed.opd[2], true, 16)
        : instr.jump;
      return Base.nop();
    },
    ...
    "fill-array-data": function (instr, context, instructions) {
      ...
      if (!dst) {
        var addr = Long.from(
          instr.parsed.opd[1],
          true,
          instr.parsed.opd[1].startsWith("0x") ? 16 : 10,
        );
        var reference = Global().xrefs.find_string(addr) ||
          Global().xrefs.find_symbol(addr);
        if (!reference) {
      ...
      return Base.special(
        "for (int32_t i = 0; " + src + ".length; i++) " + src + " = " + dst +
          "[i]",
      );
    },
    nop: function () {
    ...
    "check-cast": function (instr, context, instructions) {
      ...
      return Base.throw(
        Variable.object(
          ...
          _throw_exception(instr.parsed.opd[1]),
        ),
      );
    },
    ...
    "goto": function (instr, context, instructions) {
      instr.jump = Long.from(instr.parsed.opd[0], true, 16);
  ...
  parse: function (assembly) {
    const regex =
      /^([\w-]+)(\/?(from|high|range|jumbo|2addr|lit\d+)?(\d+)?(\s.+$)$)?(\s.+$)?$/;
    ...
    operands = token[5]
      ? token[5].trim().replace(/;\s+0x[\da-f]+(\s+)?$|^\{.+\},|^\{\},/g, "")
        .trim().split(", ")
      : [];
    args = token[5] && token[5].match(/({.+})|({})/)
      ? token[5].match(/({.+})|({})/)[0].replace(/[{,}]/g, " ").replace(
        /\s+/g,
        " ",
      ).trim().split(" ")
      : [];
  ...
  routine_name: function (name) {
    return Extra.replace.object(
      name.replace(/_([BCDFIJSVZ]+)?_[BCDFIJSVZ]$/, ""),
    ).replace(/_/g, ".");
  },
};
