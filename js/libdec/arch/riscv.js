// SPDX-FileCopyrightText: 2021-2023 Giovanni Dante Grazioli <deroad@libero.it>
...
function lui32(instr, start, instructions, context) {
  ...
  var check = [
    ...
    function (e, r) {
      ...
      return (e.mnem == "ori" &&
        (e.opd[0] == r || e.opd[0].indexOf("a") == 0) && e.opd[1] == r) ||
        (e.mnem == "addi" && (e.opd[0] == r || e.opd[0].indexOf("a") == 0) &&
          e.opd[1] == r) ||
        (e.mnem == "addiu" && (e.opd[0] == r || e.opd[0].indexOf("a") == 0) &&
          e.opd[1] == r);
    },
  ];
  var address = [
...
export default {
  instructions: {
    ...
    jal: function (instr, context, instructions) {
      ...
      for (i = instructions.indexOf(instr) - 1; i >= 0; i--) {
        if (
          instructions[i].parsed.mnem.startsWith("b") ||
    ...
    fence: function () {
      return Base.macro(
        "memory_fence()",
        "#define memory_fence() __asm(fence)",
      );
    },
    "fence.i": function () {
      return Base.macro(
        "memory_fence_i()",
        "#define memory_fence_i() __asm(fence.i)",
      );
    },
    "sfence.vma": function () {
      return Base.macro(
        "memory_sfence_vma()",
        "#define memory_sfence_vma() __asm(sfence.vma)",
      );
    },
    ebreak: function () {
      return Base.macro(
        "environment_break()",
        "#define environment_break() __asm(ebreak)",
      );
    },
    ecall: function () {
      return Base.macro(
        "environment_call()",
        "#define environment_call() __asm(ecall)",
      );
    },
    wfi: function () {
      return Base.macro(
  ...
  parse: function (asm) {
    asm = asm.replace(/,/g, " ").replace(/\s+/g, " ").trim().split(" ").map(
      function (x) {
        if (x == "zero") {
          return "0";
        }
