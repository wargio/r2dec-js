...
var _post_analysis = function (session, arch, arch_context) {
  ...
  var routine_name = arch.routine_name
    ? arch.routine_name(session.routine_name)
    : Extra.replace.call(session.routine_name);
...
var _decompile = function (session, arch, arch_context) {
  ...
  for (var i = 0; i < instructions.length; i++) {
    ...
    instr.code = fcn
      ? fcn(instr, arch_context, instructions)
      : new Base.unknown(instr.assembly);
  }
};
...
var _print = function (session) {
  ...
  if (Global().evars.extra.ascomment) {
    session.ascomment();
    Global().context.printLog(
      '[r2dec] comments applied for "' + session.routine_name + '".',
    );
    return;
  } else if (Global().evars.extra.ascodeline) {
    session.ascodeline();
    Global().context.printLog(
      '[r2dec] new code lines applied for "' + session.routine_name + '".',
    );
    return;
  }
  if (!Global().evars.extra.allfunctions) {
    ...
    var details = "/* " + Global().evars.extra.file + " @ 0x" +
      Global().evars.extra.offset.toString(16) + " */";
    var r2version = "";
    ...
    if (Global().evars.extra.annotation) {
      Global().context.addAnnotation(
        Anno.comment("/* r2dec pseudo code output" + r2version + " */\n"),
      );
      Global().context.addAnnotation(Anno.comment(details + "\n"));
    } else {
      Global().context.printLine(
        Global().context.identfy(asm_header.length, t.comment(asm_header)) +
          t.comment("/* r2dec pseudo code output" + r2version + " */"),
      );
      Global().context.printLine(
        Global().context.identfy() + t.comment(details),
      );
    }
...
var _session = function (data, arch) {
  ...
  Global().context.identAsmSet(
    max_address +
      (max_length < this.routine_name.length
        ? this.routine_name.length
        : max_length),
  );
